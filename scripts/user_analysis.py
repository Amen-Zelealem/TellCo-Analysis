...
class UserBehaviorAnalysis:
    ...
    #         total_dl=('Total Download (Bytes)', 'sum'),  # Sum total download data in bytes
    #         total_ul=('Total Upload (Bytes)', 'sum')   # Sum total upload data in bytes
    #     )
...
def aggregate_user_behavior(self):
    ...
    user_behavior["total_data_volume"] = (
        user_behavior["total_dl"] + user_behavior["total_ul"]
    )
    ...
    def segment_users_by_decile(self):
        ...
        user_behavior["total_duration"] = pd.to_numeric(
            user_behavior["total_duration"], errors="coerce"
        )
        user_behavior["total_data_volume"] = pd.to_numeric(
            user_behavior["total_data_volume"], errors="coerce"
        )

        # Create deciles based on total duration, assigning labels while handling duplicates
        user_behavior["Decile"] = pd.qcut(
            user_behavior["total_duration"], 10, labels=False, duplicates="drop"
        )
        ...
        decile_summary = (
            top_five_deciles.groupby("Decile")
            .agg(
                {
