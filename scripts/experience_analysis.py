...
class ExperienceAnalytics:
    ...
    def aggregate_user_experience(self):
        ...
        self.df['Avg RTT DL (ms)'].fillna(self.df['Avg RTT DL (ms)'].mean(), inplace=True)
        self.df['Avg RTT UL (ms)'].fillna(self.df['Avg RTT UL (ms)'].mean(), inplace=True)
        self.df['Avg Bearer TP DL (kbps)'].fillna(self.df['Avg Bearer TP DL (kbps)'].mean(), inplace=True)
        self.df['Avg Bearer TP UL (kbps)'].fillna(self.df['Avg Bearer TP UL (kbps)'].mean(), inplace=True)
    ...
    def get_top_bottom_most_frequent(self, column):
        ...
        most_frequent = df[column].mode().iloc[0] if not df[column].mode().empty else None  # Most frequent value
    ...
    def avg_tcp_rtt_per_handset(self):
        ...
        return self.df.groupby('Handset Type')['TCP_Retransmission'].mean().reset_index()
